    ...
    $.extend(Plugin.prototype, {
        ...
        ,renderList:function () {
            ...
            if(that.settings.isSetCookies)
                cookiesData = getProjectParamCookies(that._cookiesMark,that._currentCompanyUserId);

            if(!isNullOrBlank(cookiesData) && that.settings.isFirstLoad){
                option.postData = cookiesData.param;
            }
            if(isNullOrBlank(option.postData.projectNoOrder)){
                option.postData.projectNoOrder ='1';
            }
            m_ajax.postJson(option,function (response) {
                if(response.code=='0'){
                    ...
                    if(that.settings.isSetCookies){
                        //通过Cookie记住项目页码
                        option.postData.projectNoOrder='';
                        var $cookiesData = {
                            id:that._currentCompanyUserId,
                    ...
                    var html = template('m_field/m_field_list_row',{
                        ...
                        trClass:that.settings.trClass
                    });
                    $(that.element).html(html);

                    if(that.settings.renderCallBack)
                        that.settings.renderCallBack(response.data,option.param);
        ...
        ,renderListByPagination:function () {
            ...
            option.param.type = that.settings.projectType;
            ...
            if(isNullOrBlank(option.param.projectNoOrder)){
                option.param.projectNoOrder ='1';
            }
            paginationFun({

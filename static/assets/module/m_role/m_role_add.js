/**
 * 角色添加
 * Created by wrb on 2018/4/24.
 */
;(function ($, window, document, undefined) {

    "use strict";
    var pluginName = "m_role_add",
        defaults = {
            title:null,
            isDialog:true,
            doType:1,//1=个人权限，2=部门权限
            dataInfo:null,//当此值不为null则是编辑
            saveCallBack:null//保存回掉方法
    ...
    $.extend(Plugin.prototype, {
        init: function () {
            var that = this;
            var html = template('m_role/m_role_add',{dataInfo:that.settings.dataInfo,doType:that.settings.doType});
            that.renderDialog(html,function () {
                that.getRoleGroup();
                that.saveRole_validate();
            });
        },
        //初始化数据
        renderDialog:function (html,callBack) {
            ...
            if(that.settings.isDialog===true){//以弹窗编辑
                ...
                S_layer.dialog({
                    title: that.settings.title||'新增角色',
                    area : '750px',
                    content:html,
                    cancel:function () {
                    },
        ...
        ,getRoleGroup:function () {
            var that = this;
            var options={};
            options.url= restApi.url_listCustomRoleGroup;
            options.postData = {};
            options.postData.groupType = that.settings.doType==1?1:0;
        ...
        ,saveRole:function () {
            ...
            $data.roleType = that.settings.doType==1?3:4;
            var options={};
            options.url= restApi.url_saveRole;

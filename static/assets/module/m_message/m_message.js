...
;(function ($, window, document, undefined) {
    ...
    $.extend(Plugin.prototype, {
        ...
        , _renderEmptyHtml: function (data) {
            ...
            var iHtml = template('m_message/m_message_empty', {});
        ...
        , _bindBtnHandle: function () {
            ...
            $(that.element).find('.msg-content a').click(function () {
                ...
                var sendCompanyId = $this.closest('.feed-element').attr('data-send-companyId');
                ...
                if(isNullOrBlank(sendCompanyId))
                    sendCompanyId = window.currentCompanyId;
                ...
                switch (messageType) {
                    ...
                    case '303':
                    case '305':
                        //url = _.sprintf('%s/iWork/home/workbench#/project/basicInfo?id=%s&projectName=%s', window.rootPath,projectId,encodeURI(projectName));
                        url = _.sprintf('%s/iWork/home/workbench#/project/taskIssue?id=%s&projectName=%s&dataCompanyId=%s', window.serverPath,projectId,encodeURI(projectName),companyId);
                        gotoPage(url);
                        break;
                    case '401':
                    case '402':
                        url = _.sprintf('%s/iWork/home/workbench#/project/production?id=%s&projectName=%s&dataCompanyId=%s', window.serverPath,projectId,encodeURI(projectName),companyId);
                        gotoPage(url);
                        break;
                    case '403':
                    case '404':
                    ...
                    case '802':
                    case '803':
                        url = _.sprintf('%s/iWork/home/workbench#/personalSettings', window.serverPath);
                        gotoPage(url);
                        break;
                }

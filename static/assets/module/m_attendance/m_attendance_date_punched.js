...
;(function ($, window, document, undefined) {
    ...
    "use strict";
    var pluginName = "m_attendance_date_punched",
        defaults = {
            isDialog:true,
            dataType:1,//1=必须打卡的日期,0=不用打卡的日期
    ...
    $.extend(Plugin.prototype, {
        ...
        , render: function () {
            ...
            //获取当前编辑数据对象
            var dataItem = {};
            if(that.settings.id!=null && that.settings.dataInfo!=null)
                dataItem = getObjectInArray(that.settings.dataInfo,that.settings.id,'id');

            var html = template('m_attendance/m_attendance_date_punched', {
            ...
            if(that.settings.isDialog===true){//以弹窗编辑
                ...
                S_layer.dialog({
                    ...
                    ok:function () {
                        ...
                        if(that.settings.dataType==1 && $(that.element).find('.attendance-time-box label.error').length>0){
                        ...
                        data.id = that.settings.id || UUID.genV4().hexNoDelim;

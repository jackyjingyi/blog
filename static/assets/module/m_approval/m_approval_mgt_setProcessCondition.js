    ...
    $.extend(Plugin.prototype, {
        ...
        ,renderICheck:function () {
            ...
            var ifClicked = function (e) {
                var name = $(this).closest('label.i-checks').attr('data-name');
                var unit = $(this).closest('label.i-checks').attr('data-unit');
                $(that.element).find('#processName').html(name);
                if(!isNullOrBlank(unit)){
                    $(that.element).find('#processUnit').html('('+unit+')');
                }else{
                    $(that.element).find('#processUnit').html('');
                }
            ...
            $(that.element).find('.i-checks').eq(0).click();
        }
        //渲染列表内容
            ...
            that.renderDialog(html,function () {
                ...
                if(that.settings.processData && that.settings.processData.flowTaskGroupList!=null && that.settings.processData.flowTaskGroupList.length>0){
                    $.each(that.settings.processData.flowTaskGroupList,function (i,item) {
                        if(i>0 && i<that.settings.processData.flowTaskGroupList.length-1){
        ...
        ,saveProcessCondtion:function () {
            ...
            option.url = restApi.url_prepareSetProcessCondition;
            option.postData = {};
            option.postData.key = that.settings.processData.key;
            option.postData.type = that.settings.type;
            option.postData.conditionFieldId =  $(that.element).find('input[name="optionalCondition"]:checked').val();
            var list = [];
            ...
            m_ajax.postJson(option, function (response) {
                if (response.code == '0') {
                    ...
                    var optionalCondition = option.postData.conditionFieldId ;
                    if(that.settings.oKCallBack)
                        that.settings.oKCallBack(response.data,optionalCondition);
        ...
        , submit_validate: function () {
            ...
            $(that.element).find('form.form-horizontal').validate({
                ...
                errorPlacement: function (error, element) {  //重新编辑error信息的显示位置
                    error.appendTo(element.closest('.form-group').next('.error-tip'));
                }
            });

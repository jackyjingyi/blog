    ...
    $.extend(Plugin.prototype, {
        init: function () {
            ...
            $(that.element).find('#selectOrg').m_org_chose_byTree({
                ...
                param : {
                    permissionCode:'400010',
                    managerPermissionCode:'40001001'
                },
        ...
        , renderFeeEntry:function (expDate) {
            ...
            m_ajax.get(option,function (response) {
                if(response.code=='0'){
                    ...
                    var html = template('m_finance/m_feeEntry_content',{
                        expFixedData:response.data,
                    ...
                    rolesControl();
                    new leftBoxHeightResize($(that.element).find('#left-out-box'),$(that.element).find('#left-box'),$(that.element).find('#right-box'),112,false).init();
                }else {
            ...
            $(that.element).find('form#expFixedForm').validate({
                ...
                messages: {
                    expAmount:{
                        number:'请输入有效数字',
                        minNumber:'请输入大于0的数字!',
                        over10:'对不起，您的操作超出了系统允许的范围。合同总金额的单位为“元”',
                        ckFee:'请保留小数点后两位!'
                    }
                },
